          ...
          <article key={dossier.trainer} className="trainer-dossier-card">
            <div className="trainer-dossier-header">
              ...
              <div className="trainer-dossier-copy">
                ...
                <p className="muted">{dossier.region} | {dossier.sourceGames[0]}</p>
                ...
                {dossier.source ? (
                  <a className="source-link" href={dossier.source.url} target="_blank" rel="noreferrer">
            ...
            <div className="preset-stat-grid">
              <div><strong>{dossier.uniqueTypeCount}</strong><span>Types used</span></div>
              <div><strong>{dossier.teamMemberCount}</strong><span>Roster slots</span></div>
              <div><strong>{dossier.totalMoveGroups}</strong><span>Move groups</span></div>
              <div><strong>{dossier.specialPokemonCount}</strong><span>Legend/Mythic</span></div>
            </div>
            <p className="small-copy"><strong>Battle roles:</strong> {dossier.categories.join(", ")}</p>
            <p className="small-copy"><strong>Appearances:</strong> {dossier.battleLabels.join(", ")}</p>
            <p className="small-copy"><strong>Archive tags:</strong> {dossier.tags.join(", ")}</p>
          </article>
        ))}
      </div>
    </section>
